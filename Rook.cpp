...
vector<Move> Rook::set_moves(bool t){
	...
	if(is_color_set && is_pos_set && !is_null_piece){
		...
		if(X < 7){
			Move t1(X+1, Y, color, true, false, ROOK, x, y); //gaurding friendly
		...
		if(Y < 7){
			Move t2(X, Y+1, color, true, false, ROOK, x, y);
		...
		if(X > 0){
			Move t3(X-1, Y, color, true, false, ROOK, x, y);
		...
		if( is_enemy_piece_type(pos->the_board[X][Y-1]) ){
			...
			Move t4(X, Y-1, color, true, false, ROOK, x, y); 
