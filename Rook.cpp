...
vector<Move> Rook::set_moves(bool t){
	...
	if(is_color_set && is_pos_set && !is_null_piece){
		...
		while(X < 7 && pos->the_board[X+1][Y] == NULLCELL){
			Move m_down(X+1, Y, color, false, false, ROOK, x, y, false, false,false);
			moves.push_back(m_down);
			threats.push_back(m_down);
			X++;
		}
		if(X < 7 && is_enemy_piece_type(pos->the_board[X+1][Y]) ){
			Move t_down(X+1, Y, color, true, false, ROOK, x, y, false, false, false);
			moves.push_back(t_down);
			threats.push_back(t_down); 
		}else if(X < 7 && !is_enemy_piece_type(pos->the_board[X+1][Y])){					  Move t_down(X+1, Y, color, true, false, ROOK, x, y, false, false, false);
			threats.push_back(t_down); 
		}
		...
		while(Y < 7 && pos->the_board[X][Y+1] == NULLCELL){
			Move m_right(X, Y+1, color, false, false, ROOK, x, y, false, false, false);
		...
		if(Y < 7 && is_enemy_piece_type(pos->the_board[X][Y+1]) ){
			Move t_right(X, Y+1, color, true, false, ROOK, x, y, false, false, false);
		...
		while(X > 0 && pos->the_board[X-1][Y] == NULLCELL){
			Move m_up(X-1, Y, color, false, false, ROOK, x, y, false, false, false);
			moves.push_back(m_up);
			threats.push_back(m_up);
			X--;
		}
		if(X > 0 && is_enemy_piece_type(pos->the_board[X-1][Y]) ){
			Move t_up(X-1, Y, color, true, false, ROOK, x, y, false, false, false);
